...
import { InvalidQuantityException, NotFoundCartItemException, NotFoundSKUException, OutOfStockSKUException, ProductNotFoundException } from "./cart.error";
...
export class CartRepo {
    ...
    private async validateSKU({ skuId, quantity, userId, isCreate }: { skuId: number, quantity: number, userId: number, isCreate: boolean }): Promise<SKUSchemaType> {
        ...
        const { product } = sku
        // kiểm tra sản phẩm đã bị xoá hoặc có công khai hay không
    ...
    async list({ userId, languageId, limit, page }: {
        userId: number
        languageId: string,
        limit: number,
        page: number
   ...
   `
    ...
    async create(userId: number, body: AddToCartBodyType): Promise<CartItemType> {
        await this.validateSKU({ isCreate: true, userId, skuId: body.skuId, quantity: body.quantity })

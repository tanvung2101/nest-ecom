...
export class ReviewRepository {
    ...
    async create(userId: number, body: CreateReviewBodyType): Promise<CreateReviewResType>{
        ...
        return this.prismaService.$transaction(async (tx) => {
            const review = await tx.review.create({
                data: {
                    content, rating, productId, orderId, userId
                },
                include: {
                    user: {
                        select: {
    ...
    async update({userId, reviewId, body}: {userId: number, reviewId: number, body: UpdateReviewBodyType}):Promise<UpdateReviewResType>{
        const {content, medias, productId, orderId, rating} = body

        await Promise.all([
            this.validateOrder({orderId,userId}),
            this.validateUpdateReview({reviewId, userId})
        ])
